package Praktikum12
  ...
  equation
    ...
    connect(y, gain.y) annotation (Line(points={{-106,-48},{-78,-48},{-78,24},{
            -73.3,24},{-73.3,43}}, color={0,0,127}));
    annotation (
      Icon(coordinateSystem(preserveAspectRatio = false)),
      Diagram(coordinateSystem(preserveAspectRatio = false)));
...
(will be replaced by a color editor).
        ...
        model CoordinateSystem "Visualizing an orthogonal coordinate system of three axes"
          ...
          parameter SI.Length scaledLabel = PlanarMechanics.Types.Defaults.FrameLabelHeightFraction * axisDiameter annotation (
            Dialog(group = "Axes labels", enable = axisShowLabels));
        protected
          outer .Praktikum12.PlanarMechanics.PlanarWorld planarWorld;
...
<b>Developed 2010-2019 at the DLR Institute of System Dynamics and Control</b>
  ...
  Praktikum12.Differential differential annotation (Placement(visible=true,
          transformation(
          origin={62,30},
          extent={{-10,-10},{10,10}},
          rotation=90)));
  ...
  connect(differential.flange_b, VR) annotation (
    ...
    connect(differential1.flange_b, HR) annotation (Line(points={{60,-50},{82,
            -50},{82,-60},{100,-60},{100,-60}}, color={0,0,0}));
    connect(differential1.flange_b1, HL) annotation (Line(points={{60,-30},{80,
            -30},{80,-20},{100,-20}}, color={0,0,0}));
  annotation (
      Icon(coordinateSystem(preserveAspectRatio = false)),
      Diagram(coordinateSystem(preserveAspectRatio = false)));
  end DriveTrain2D;
  ...
  model Chassis2D
    ...
    Praktikum12.PlanarMechanics.VehicleComponents.Wheels.DryFrictionWheelJoint dryFrictionWheelJoint1(
      radius=wheelRadius,
      r={1,0},
      N=(carMass/4 + 19)*9.81,
      vAdhesion=0.01,
    ...
    Praktikum12.PlanarMechanics.Parts.FixedTranslation fixedTranslation1(r={0,-
          trackWidth/2}) annotation (Placement(visible=true, transformation(
          origin={48,20},
          extent={{10,-10},{-10,10}},
          rotation=0)));
    Praktikum12.PlanarMechanics.Parts.FixedTranslation fixedTranslation3(r={0,-
          trackWidth/2}) annotation (Placement(visible=true, transformation(
          origin={46,-60},
          extent={{10,-10},{-10,10}},
          rotation=0)));
    Praktikum12.PlanarMechanics.Parts.Body body1(m=19, I=0.653) annotation (
        Placement(visible=true, transformation(
          origin={-20,42},
          extent={{-8,-8},{8,8}},
          rotation=90)));
    Praktikum12.PlanarMechanics.Parts.Body body3(
      I=0.653,
      animate=true,
      enableGravity=true,
    ...
    Praktikum12.PlanarMechanics.Parts.FixedTranslation fixedTranslation5(r={
          wheelBase/2,0}) annotation (Placement(visible=true, transformation(
          origin={80,36},
          extent={{10,-10},{-10,10}},
          rotation=-90)));
    Praktikum12.PlanarMechanics.Joints.Revolute revolute(useFlange=true, phi(
    ...
    Praktikum12.PlanarMechanics.Parts.FixedTranslation fixedTranslation2(r={0,
          trackWidth/2}) annotation (Placement(visible=true, transformation(
          origin={46,-20},
          extent={{10,-10},{-10,10}},
          rotation=0)));
...
equation
  ...
  model SteeringLimiterTest
    Praktikum12.SteeringLimiter steeringLimiter
...
<strong>end for</strong>;
    ...
    equation
      ...
      connect(fixedTranslation3.frame_a, fixedTranslation.frame_b) annotation (
          Line(
          points={{60,16},{60,44},{82,44},{82,36}},
          color={95,95,95},
          thickness=0.5));
      ...
      connect(bodyShape5.frame_a, dryFrictionWheelJoint3D2.frame_wheel)
        annotation (Line(
          points={{-64,-66},{-50,-66}},
          color={95,95,95},
          thickness=0.5));
...
<b>Developed 2010-2019 at the DLR Institute of System Dynamics and Control</b>
    ...
    equation
      ...
      -f_long*radius = flange_a.tau;
      frame_a.t = {0,0,0};
      f = N*noEvent(Praktikum12.HybridCar3D.limitByStriple(
            vAdhesion,
            vSlide,
            mu_A,
            mu_S,
...
<b>Developed 2010-2019 at the DLR Institute of System Dynamics and Control</b>
    ...
    model DryFrictionWheelJoint3D
      ...
      Praktikum12.HybridCar3D.PartialDryFrictionWheelJoint3D partialDryFrictionWheelJoint3D(
        radius=radius,
        r={1,0},
        vAdhesion=vAdhesion,
        vSlide=vSlide,
    ...
    model HybrdiCar3D
      ...
      CombustionEngineDrive combustionEngineDrive(omegaStart = 0) annotation (
        Placement(transformation(extent={{-34,-32},{-14,-12}})));
      DriveTrain2D driveTrain2D annotation (
      ...
      Body3D body3D(wheelRadius=0.31)
        annotation (Placement(transformation(extent={{72,-18},{92,2}})));
      Chassis3D chassis3D(
        wheelBase=2.578,
        trackWidth=1.549,
        wheelRadius=0.31,
    ...
    equation
      connect(torque.flange, steeringLimiter.flange_a)
        annotation (Line(points={{-12,56},{18,56}},
                                                  color={0,0,0}));
      connect(driver.steering_angle, torque.tau) annotation (Line(points={{-71.2,
              5.6},{-71.2,56},{-34,56}}, color={0,0,127}));
      connect(chassis3D.frame, body3D.frame) annotation (Line(points={{60.2,
              -8.4},{62.05,-8.4},{62.05,-7.8},{74.2,-7.8}},
                                                          color={0,0,0}));
      connect(chassis3D.HR, driveTrain2D.HR) annotation (Line(points={{42,-16.4},
              {38,-16.4},{38,-14},{36,-14}},
                                          color={0,0,0}));
      connect(chassis3D.HL, driveTrain2D.HL) annotation (Line(points={{42,-13},
              {38,-13},{38,-10},{36,-10}},
                                       color={0,0,0}));
      connect(chassis3D.VR, driveTrain2D.VR)
        annotation (Line(points={{42,-8},{38,-8},{38,-6},{36,-6}},
                                                 color={0,0,0}));
      connect(chassis3D.VL, driveTrain2D.VL)
        annotation (Line(points={{42,-4},{38,-4},{38,-2},{36,-2}},
                                                 color={0,0,0}));
      connect(body3D.speed, driver.car_speed) annotation (Line(points={{92.6,
              -12.6},{90,-12.6},{90,-38},{-92,-38},{-92,-3},{-88.4,-3}},
                                                                       color={0,
              0,127}));
      connect(body3D.angle_Deg, driver.driving_angle_deg) annotation (Line(
            points={{90.2,0.2},{96,0.2},{96,-50},{-96,-50},{-96,8.8},{-86.8,8.8}},
            color={0,0,127}));
      connect(steeringLimiter.flange_b, chassis3D.Steering) annotation (Line(
            points={{38,56},{52,56},{52,2},{57.4,2}},   color={0,0,0}));
      connect(differential.flange_a, driveTrain2D.flange_a) annotation (Line(
            points={{-6,6.2},{-6,-22},{8,-22},{8,-8},{16,-8}}, color={0,0,0}));
      connect(combustionEngineDrive.flange_a, driveTrain2D.flange_a)
        annotation (Line(points={{-14,-22},{8,-22},{8,-8},{16,-8}}, color={0,0,
              0}));
      connect(differential.flange_b1, electricDrive.flange_a)
        annotation (Line(points={{-16,16},{-20.2,16}}, color={0,0,0}));
      connect(differential.flange_b, fixed.flange) annotation (Line(points={{4,
              16},{12,16},{12,28},{16,28}}, color={0,0,0}));
      connect(hybridStrategy.electricdrive_out, electricDrive.u) annotation (
          Line(points={{-41.4,0},{-40.4,0},{-40.4,16}}, color={0,0,127}));
      connect(hybridStrategy.y1, combustionEngineDrive.u) annotation (Line(
            points={{-41.4,-8.2},{-41.4,-23.1},{-34.2,-23.1},{-34.2,-22}},
            color={0,0,127}));
      connect(driver.car_torque, hybridStrategy.torque) annotation (Line(points
            ={{-73.6,0},{-68,0},{-68,-2.4},{-62.4,-2.4}}, color={0,0,127}));
      connect(combustionEngineDrive.y, hybridStrategy.angularvelocity)
        annotation (Line(points={{-34.6,-26.8},{-66,-26.8},{-66,-8.2},{-62.4,
              -8.2}}, color={0,0,127}));
      annotation (
        Icon(coordinateSystem(preserveAspectRatio = false)),
        Diagram(coordinateSystem(preserveAspectRatio = false)),
    ...
    model WheelSuspensionRight
      ...
      Modelica.Mechanics.MultiBody.Joints.Revolute revolute1(n(displayUnit="1")=
             {1,0,0}, phi(start=0, fixed=false))  annotation (Placement(
            transformation(
            extent={{-10,-10},{10,10}},
            rotation=180,
            origin={48,-78})));
      Modelica.Mechanics.MultiBody.Joints.Revolute revolute4(n(displayUnit="1")=
             {1,0,0}, phi(fixed=false, start=0))               annotation (
          Placement(transformation(
            extent={{-10,-10},{10,10}},
            rotation=180,
            origin={-24,-8})));
      ...
      Modelica.Mechanics.MultiBody.Joints.Prismatic prismatic(
        ...
        s(start=sqrt(0.35^2 + 0.35^2), fixed=true),
        v(start=0),
        a(start=0)) annotation (Placement(transformation(
            extent={{-10,-10},{10,10}},
            rotation=180,
            origin={8,40})));
      Modelica.Mechanics.MultiBody.Forces.Spring spring(c=100000,s_unstretched=
            sqrt(0.35^2 + 0.35^2))                             annotation (
          Placement(transformation(
            extent={{-10,-10},{10,10}},
            rotation=180,
            origin={8,64})));
      Modelica.Mechanics.MultiBody.Forces.Damper damper(d=10000)
                                                               annotation (
          Placement(transformation(
            extent={{-10,-10},{10,10}},
            rotation=180,
      ...
      Modelica.Mechanics.MultiBody.Joints.RevolutePlanarLoopConstraint revolute2(n(
            ...
            rotation=180,
            origin={-18,-78})));
      Modelica.Mechanics.MultiBody.Parts.FixedTranslation fixedTranslation6(r={0,0,
            0.15})     annotation (Placement(transformation(
            extent={{-10,-10},{10,10}},
            rotation=90,
            origin={-50,-24})));
    equation
      ...
      connect(fixedTranslation6.frame_a, fixedTranslation4.frame_b) annotation (
         Line(
          ...
          color={95,95,95},
          thickness=0.5));
      connect(fixedTranslation6.frame_b, revolute6.frame_b) annotation (Line(
          points={{-50,-14},{-50,-2}},
          color={95,95,95},
          thickness=0.5));
      connect(revolute4.frame_b, revolute6.frame_b) annotation (Line(
          points={{-34,-8},{-50,-8},{-50,-2}},
          color={95,95,95},
          thickness=0.5));
    ...
    model WheelSuspensionLeft
      ...
      Modelica.Mechanics.MultiBody.Joints.Revolute revolute1(n(displayUnit="1")=
            ...
            origin={48,-78})));
      Modelica.Mechanics.MultiBody.Joints.Revolute revolute4(n(displayUnit="1")=
             {1,0,0}, phi(start=0, fixed=false))               annotation (
          Placement(transformation(
            extent={{-10,-10},{10,10}},
            rotation=180,
            origin={-24,-8})));
    ...
    model HybridStrategy
      Modelica.Blocks.Interfaces.RealOutput electricdrive_out
        annotation (Placement(transformation(extent={{96,50},{116,70}})));
      Modelica.Blocks.Interfaces.RealOutput y1
        annotation (Placement(transformation(extent={{96,-32},{116,-12}})));
      Modelica.Blocks.Interfaces.RealInput angularvelocity
        annotation (Placement(transformation(extent={{-124,-42},{-84,-2}})));
      Modelica.Blocks.Interfaces.RealInput torque
        annotation (Placement(transformation(extent={{-124,16},{-84,56}})));
      Modelica.Blocks.Math.Gain gain(k=1/2500)
    ...
    equation
      connect(angularvelocity, gain.u)
        annotation (Line(points={{-104,-22},{-64,-22}}, color={0,0,127}));
      connect(gain.y, limiter.u)
        annotation (Line(points={{-41,-22},{-24,-22}}, color={0,0,127}));
      connect(realExpression.y, add.u1)
        annotation (Line(points={{-13,30},{2,30}}, color={0,0,127}));
      connect(add.u2, limiter.y) annotation (Line(points={{2,18},{-2,18},{-2,-4},
              {4,-4},{4,-22},{-1,-22}}, color={0,0,127}));
      connect(torque, product.u1) annotation (Line(points={{-104,36},{-42,36},{
              -42,66},{32,66}}, color={0,0,127}));
      connect(product.u2, add.y) annotation (Line(points={{32,54},{30,54},{30,24},
              {25,24}}, color={0,0,127}));
      connect(product.y, electricdrive_out)
        annotation (Line(points={{55,60},{106,60}}, color={0,0,127}));
      connect(y1, product1.y)
        annotation (Line(points={{106,-22},{57,-22}}, color={0,0,127}));
      connect(limiter.y, product1.u2) annotation (Line(points={{-1,-22},{16,-22},
              {16,-28},{34,-28}}, color={0,0,127}));
      connect(torque, product1.u1) annotation (Line(points={{-104,36},{-72,36},{
              -72,4},{26,4},{26,-16},{34,-16}}, color={0,0,127}));
      annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
            coordinateSystem(preserveAspectRatio=false)));
    end HybridStrategy;
  end HybridCar3D;
