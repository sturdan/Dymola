...
package Praktikum8
  ...
  model Driver
    ...
    Modelica.Blocks.Sources.CombiTimeTable combiTimeTable(
      tableOnFile=true,
      tableName="Cycle",
      fileName="C:/Users/sturmdan/Documents/Dymola/sort1_steering.txt",
      columns={2,3},
    ...
    Modelica.Blocks.Math.Gain gain1(k=0.5)
      annotation (Placement(transformation(extent={{50,46},{70,66}})));
    Modelica.Blocks.Math.Feedback feedback1
      annotation (Placement(transformation(extent={{10,66},{30,46}})));
    Modelica.Blocks.Interfaces.RealInput driving_angle
      annotation (Placement(transformation(extent={{-48,72},{-24,96}})));
    Modelica.Blocks.Math.Gain gain2(k=3.14/180)
      annotation (Placement(transformation(extent={{-38,44},{-18,64}})));
    Modelica.Blocks.Interfaces.RealOutput steering_angle
      annotation (Placement(transformation(extent={{78,46},{98,66}})));
  equation
    ...
    connect(combiTimeTable.y[1], KmhMs.u) annotation (Line(points={{-75,58},{
            -76,58},{-76,30},{-72,30}}, color={0,0,127}));
    connect(feedback1.u2, driving_angle) annotation (Line(points={{20,64},{-4,
            64},{-4,84},{-36,84}}, color={0,0,127}));
    connect(combiTimeTable.y[2], gain2.u) annotation (Line(points={{-75,58},{
            -58,58},{-58,54},{-40,54}}, color={0,0,127}));
    connect(gain2.y, feedback1.u1) annotation (Line(points={{-17,54},{-2.5,54},
            {-2.5,56},{12,56}}, color={0,0,127}));
    connect(feedback1.y, gain1.u)
      annotation (Line(points={{29,56},{48,56}}, color={0,0,127}));
    connect(gain1.y, steering_angle)
  ...
  equation
    ...
    connect(driveTrain.HL, chassis1D.flange_a) annotation (Line(points={{16,0},
            {22,0},{22,-0.2},{28.6,-0.2}}, color={0,0,0}));
  ...
  package PlanarMechanics
    "Library to model 2-dimensional, planar mechanical systems"
...
<b>Developed 2010-2019 at the DLR Institute of System Dynamics and Control</b>
        ...
        model SpurGear "Rigid spur gear"
          ...
          PlanarMechanics.Parts.Body gearA(m=1, I=1e-3) annotation (Placement(
                transformation(
                extent={{-10,-10},{10,10}},
                rotation=90,
                origin={-20,30})));
          PlanarMechanics.Parts.Fixed fixed_A annotation (Placement(
                transformation(
                extent={{-10,-10},{10,10}},
                rotation=270,
                origin={-70,-52})));
          PlanarMechanics.Parts.Body gearB(m=1, I=1e-3) annotation (Placement(
                transformation(
                extent={{-10,-10},{10,10}},
                rotation=90,
                origin={20,30})));
        ...
        package Utilities "Utility elements used for gear example models"
          ...
          model RigidNoLossPlanetary "Planetary gearbox"
            ...
            PlanarMechanics.Parts.Body planet(
              m=1,
              I=1e-3,
              phi(fixed=false)) annotation (Placement(transformation(extent={{-10,
                      -10},{10,10}}, origin={82,40})));
            PlanarMechanics.Parts.FixedTranslation carrierPart(r={r_s + r_p,0})
              annotation (Placement(transformation(extent={{-8,-50},{12,-30}})));
            PlanarMechanics.Parts.Fixed fixed annotation (Placement(
                  transformation(
                  extent={{-10,-10},{10,10}},
                  rotation=180,
                  origin={-90,30})));
...
</html>"),Diagram(coordinateSystem(extent={{-40,-20},{40,20}}, preserveAspectRatio=false),
              graphics={Line(
                points={{-40,0},{40,0}},
                color={95,95,95},
                thickness=0.5)}));
    ...
    package Joints "Planar joint models"
      ...
      model Prismatic "A prismatic joint"
        ...
        input PlanarMechanics.Types.Color boxColor=Types.Defaults.JointColor
          "Color of prismatic joint box" annotation (HideResult=true, Dialog(
            tab="Animation",
            group="if animation = true",
            enable=animate,
            colorSelector=true));
        input PlanarMechanics.Types.SpecularCoefficient specularCoefficient=
...
<p>In &quot;Animation&quot; group, animation parameters for this model can be set, where <b>zPosition</b> represents the model&apos;s position along the z axis in 3D animation. Some of the values can be preset by an outer PlanarWorld model.</p>
      ...
      model Revolute "A revolute joint"
        ...
        input PlanarMechanics.Types.Color cylinderColor=Types.Defaults.JointColor
          "Color of cylinder representing the joint axis" annotation (
            HideResult=true, Dialog(
            colorSelector=true,
            tab="Animation",
            group="if animation = true",
            enable=animate));
...
<p>This component defines the x, y-position and angle of the frame connectors, to which this component is attached to.</p>
      ...
      model FixedRotation "A fixed translation between two components (rigid rod)"
        ...
        input PlanarMechanics.Types.Color color=Types.Defaults.RodColor
          "Color of cylinder representing the fixed rotation" annotation (
            HideResult=true, Dialog(
            colorSelector=true,
            tab="Animation",
            group="if animation = true",
...
<p>Package <b>Parts</b> contains rigid components and spring/damper components for planar mechanical systems, which could be used to build up a complex planar system.</p>
    ...
    model PlanarWorld
      ...
      parameter Real defaultFrameDiameterFraction=40
        ...
        annotation (Dialog(tab="Defaults"));
      parameter PlanarMechanics.Types.SpecularCoefficient defaultSpecularCoefficient(min=0)=
        0.7
...
transformed output vector as \"Real r_out[3]\";
      ...
      model CutForce "Measure cut force vector"
        ...
        input PlanarMechanics.Types.SpecularCoefficient specularCoefficient=
            planarWorld.defaultSpecularCoefficient
          "Reflection of ambient light (= 0: light is completely absorbed)"
          annotation (HideResult=true, Dialog(group="if animation = true",
              enable=animation));
      ...
      protected
        ...
        PlanarMechanics.Visualizers.Advanced.Arrow arrow(
          ...
          R=MB.Frames.absoluteRotation(planarWorld.R, MB.Frames.planarRotation(
                    ...
                    phi,
                    der(phi))),
          r=MB.Frames.resolve1(planarWorld.R, {frame_b.x,frame_b.y,zPosition})
               + planarWorld.r_0,
          r_tail=-f_in_m,
...
(will be replaced by a color editor).
        ...
        model CoordinateSystem
          ...
          parameter PlanarMechanics.Types.Color color_x=PlanarMechanics.Types.Defaults.FrameColor
            "Color of x-arrow"
            annotation (HideResult=true, Dialog(colorSelector=true));
          parameter PlanarMechanics.Types.Color color_y=color_x
            "Color of y-arrow"
            annotation (HideResult=true, Dialog(colorSelector=true));
          parameter PlanarMechanics.Types.Color color_z=color_x
            "Color of z-arrow"
            annotation (HideResult=true, Dialog(colorSelector=true));
        ...
        protected
          outer .Praktikum8.PlanarMechanics.PlanarWorld planarWorld;
      ...
      package Internal "Collection of internal visualizers, should not be used by user"
        ...
        protected
          ...
          SI.Length headWidth=noEvent(max(0, diameter*PlanarMechanics.Types.Defaults.ArrowHeadWidthFraction))
            annotation(HideResult=true);
          SI.Length arrowLength = noEvent(max(0, length - diameter*
              PlanarMechanics.Types.Defaults.ArrowHeadLengthFraction))
            annotation(HideResult=true);
...
<b>Developed 2010-2019 at the DLR Institute of System Dynamics and Control</b>
  ...
  model DriveTrain2D
    ...
    Modelica.Mechanics.Rotational.Interfaces.Flange_a flange_a
      annotation (Placement(transformation(extent={{-110,-10},{-90,10}})));
    Modelica.Mechanics.Rotational.Interfaces.Flange_b VR
      annotation (Placement(transformation(extent={{90,10},{110,30}})));
    Modelica.Mechanics.Rotational.Interfaces.Flange_b VL
      annotation (Placement(transformation(extent={{90,50},{110,70}})));
    Modelica.Mechanics.Rotational.Interfaces.Flange_b HL
      annotation (Placement(transformation(extent={{90,-30},{110,-10}})));
    Modelica.Mechanics.Rotational.Interfaces.Flange_b HR
      annotation (Placement(transformation(extent={{90,-70},{110,-50}})));
    Differential differential annotation (Placement(transformation(
          extent={{-10,-10},{10,10}},
          rotation=90,
          origin={48,40})));
  equation
    connect(idealGear.flange_a, flange_a)
      annotation (Line(points={{-6,0},{-100,0}}, color={0,0,0}));
    connect(HL, HL)
      annotation (Line(points={{100,-20},{100,-20}}, color={0,0,0}));
    connect(HR, HL) annotation (Line(points={{100,-60},{80,-60},{80,-20},{100,
            -20}}, color={0,0,0}));
    connect(differential.flange_a, idealGear.flange_b) annotation (Line(points=
            {{38.2,40},{26,40},{26,0},{14,0}}, color={0,0,0}));
    connect(differential.flange_b1, VL)
      annotation (Line(points={{48,50},{48,60},{100,60}}, color={0,0,0}));
    connect(differential.flange_b, VR)
      annotation (Line(points={{48,30},{48,20},{100,20}}, color={0,0,0}));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
          coordinateSystem(preserveAspectRatio=false)));
  end DriveTrain2D;
  ...
  model Chassis2D
    ...
    Modelica.Mechanics.Rotational.Interfaces.Flange_a HR
      annotation (Placement(transformation(extent={{-110,-70},{-90,-50}})));
    Modelica.Mechanics.Rotational.Interfaces.Flange_a VL
      annotation (Placement(transformation(extent={{-110,50},{-90,70}})));
    Modelica.Mechanics.Rotational.Interfaces.Flange_a VR
      annotation (Placement(transformation(extent={{-110,10},{-90,30}})));
    Modelica.Mechanics.Rotational.Interfaces.Flange_a HL
    ...
    PlanarMechanics.Interfaces.Frame frame
      annotation (Placement(transformation(extent={{72,-8},{88,8}})));
    PlanarMechanics.Parts.FixedTranslation fixedTranslation(r={0,trackWidth/2})
      annotation (Placement(transformation(extent={{16,50},{36,70}})));
    PlanarMechanics.VehicleComponents.Wheels.DryFrictionWheelJoint
      dryFrictionWheelJoint(radius=wheelRadius,
      r={1,0},
      N=((carMass/4) + 19)*9.81,
      vAdhesion(displayUnit="m/s") = 0.01,
      vSlide=0.1,
      mu_A=0.85,
      mu_S=0.5)
    ...
    PlanarMechanics.Parts.Body body(m=19, I=0.653)
                                               annotation (Placement(
          transformation(
          extent={{-10,-10},{10,10}},
          rotation=90,
          origin={-32,88})));
    Modelica.Mechanics.Rotational.Components.Inertia inertia(J=1.055)
      annotation (Placement(transformation(extent={{-78,50},{-58,70}})));
    PlanarMechanics.VehicleComponents.Wheels.DryFrictionWheelJoint
      dryFrictionWheelJoint1(radius=wheelRadius,
      r={1,0},
      N=((carMass/4) + 19)*9.81,
      vAdhesion=0.01,
      vSlide=0.1,
    ...
    Modelica.Mechanics.Rotational.Components.Inertia inertia1(J=1.055)
      annotation (Placement(transformation(extent={{-78,10},{-58,30}})));
    PlanarMechanics.VehicleComponents.Wheels.DryFrictionWheelJoint
      dryFrictionWheelJoint2(radius=wheelRadius,
      r={1,0},
      N=((carMass/4) + 19)*9.81,
      vAdhesion(displayUnit="m/s") = 0.01,
      vSlide(displayUnit="m/s") = 0.1,
      mu_A=0.85,
      mu_S=0.5)
      annotation (Placement(transformation(extent={{-28,-28},{-48,-8}})));
    Modelica.Mechanics.Rotational.Components.Inertia inertia2(J=1.055)
      annotation (Placement(transformation(extent={{-80,-30},{-60,-10}})));
    PlanarMechanics.VehicleComponents.Wheels.DryFrictionWheelJoint
      dryFrictionWheelJoint3(radius=wheelRadius,
      r={1,0},
      N=((carMass/4) + 19)*9.81,
      vAdhesion(displayUnit="m/s") = 0.01,
      vSlide=0.1,
    ...
    Modelica.Mechanics.Rotational.Components.Inertia inertia3(J=1.055)
      annotation (Placement(transformation(extent={{-80,-70},{-60,-50}})));
    PlanarMechanics.Parts.FixedTranslation fixedTranslation1(r={0,(-trackWidth/
          2)})
      annotation (Placement(transformation(extent={{16,10},{36,30}})));
    PlanarMechanics.Parts.FixedTranslation fixedTranslation2(r={0,trackWidth/2})
      annotation (Placement(transformation(extent={{-14,-30},{6,-10}})));
    PlanarMechanics.Parts.FixedTranslation fixedTranslation3(r={0,(-trackWidth/
          2)})
    ...
    PlanarMechanics.Parts.Body body1(m=19, I=0.653)
                                                annotation (Placement(
          transformation(
          extent={{-8,-8},{8,8}},
          rotation=90,
          origin={-24,36})));
    PlanarMechanics.Parts.Body body2(m=19, I=0.653)
                                                annotation (Placement(
          transformation(
          extent={{-8,-8},{8,8}},
          rotation=90,
          origin={-22,0})));
    PlanarMechanics.Parts.Body body3(m=19, I=0.653)
                                                annotation (Placement(
          transformation(
          extent={{-8,-8},{8,8}},
          rotation=90,
          origin={-20,-44})));
    PlanarMechanics.Parts.FixedTranslation fixedTranslation4(r={(-wheelBase)/2,
          0})
      annotation (Placement(transformation(extent={{-10,-10},{10,10}},
          rotation=-90,
          origin={80,-30})));
    PlanarMechanics.Parts.FixedTranslation fixedTranslation5(r={(-wheelBase)/2,
          0})
      annotation (Placement(transformation(extent={{-10,-10},{10,10}},
          rotation=-90,
          origin={80,36})));
  equation
    connect(VL, inertia.flange_a)
      annotation (Line(points={{-100,60},{-78,60}}, color={0,0,0}));
    connect(inertia.flange_b, dryFrictionWheelJoint.flange_a)
      annotation (Line(points={{-58,60},{-48,60}}, color={0,0,0}));
    connect(inertia1.flange_b, dryFrictionWheelJoint1.flange_a)
      annotation (Line(points={{-58,20},{-48,20}}, color={0,0,0}));
    connect(inertia1.flange_a, VR)
      annotation (Line(points={{-78,20},{-100,20}}, color={0,0,0}));
    connect(inertia2.flange_b, dryFrictionWheelJoint2.flange_a)
      annotation (Line(points={{-60,-20},{-54,-20},{-54,-18},{-48,-18}},
                                                     color={0,0,0}));
    connect(inertia2.flange_a, HL)
      annotation (Line(points={{-80,-20},{-100,-20}}, color={0,0,0}));
    connect(inertia3.flange_b, dryFrictionWheelJoint3.flange_a)
      annotation (Line(points={{-60,-60},{-50,-60}}, color={0,0,0}));
    connect(HR, HR)
      annotation (Line(points={{-100,-60},{-100,-60}}, color={0,0,0}));
    connect(HR, inertia3.flange_a)
      annotation (Line(points={{-100,-60},{-80,-60}}, color={0,0,0}));
    connect(dryFrictionWheelJoint2.frame_a, fixedTranslation2.frame_a)
      annotation (Line(
        points={{-34,-18},{-24,-18},{-24,-20},{-14,-20}},
        color={95,95,95},
        thickness=0.5));
    connect(dryFrictionWheelJoint3.frame_a, fixedTranslation3.frame_a)
      annotation (Line(
        points={{-36,-60},{-14,-60}},
        color={95,95,95},
        thickness=0.5));
    connect(dryFrictionWheelJoint1.frame_a, revolute.frame_a) annotation (Line(
        points={{-34,20},{-18,20}},
        color={95,95,95},
        thickness=0.5));
    connect(dryFrictionWheelJoint.frame_a, revolute1.frame_a) annotation (Line(
        points={{-34,60},{-18,60}},
        color={95,95,95},
        thickness=0.5));
    ...
    connect(fixedTranslation.frame_a, revolute1.frame_b) annotation (Line(
        ...
        color={95,95,95},
        thickness=0.5));
    connect(dryFrictionWheelJoint.frame_a, body.frame_a) annotation (Line(
        points={{-34,60},{-34,70},{-32,70},{-32,78}},
        color={95,95,95},
        thickness=0.5));
    connect(body1.frame_a, revolute.frame_a) annotation (Line(
        points={{-24,28},{-24,20},{-18,20}},
        color={95,95,95},
        thickness=0.5));
    connect(body2.frame_a, fixedTranslation2.frame_a) annotation (Line(
        points={{-22,-8},{-22,-20},{-14,-20}},
        color={95,95,95},
        thickness=0.5));
    connect(body3.frame_a, fixedTranslation3.frame_a) annotation (Line(
        points={{-20,-52},{-20,-60},{-14,-60}},
        color={95,95,95},
        thickness=0.5));
    connect(frame, frame)
      annotation (Line(points={{80,0},{80,0}}, color={0,0,0}));
    connect(frame, fixedTranslation4.frame_a)
      annotation (Line(points={{80,0},{80,-20}}));
    connect(frame, fixedTranslation5.frame_b)
      annotation (Line(points={{80,0},{80,26}}));
    connect(fixedTranslation.frame_b, fixedTranslation5.frame_a) annotation (
        Line(
        points={{36,60},{58,60},{58,46},{80,46}},
        color={95,95,95},
        thickness=0.5));
    connect(fixedTranslation1.frame_b, fixedTranslation5.frame_a) annotation (
        Line(
        points={{36,20},{58,20},{58,46},{80,46}},
        color={95,95,95},
        thickness=0.5));
    connect(fixedTranslation2.frame_b, fixedTranslation4.frame_b) annotation (
        Line(
        points={{6,-20},{44,-20},{44,-40},{80,-40}},
        color={95,95,95},
        thickness=0.5));
    connect(fixedTranslation3.frame_b, fixedTranslation4.frame_b) annotation (
        Line(
        points={{6,-60},{44,-60},{44,-40},{80,-40}},
        color={95,95,95},
        thickness=0.5));
    connect(revolute.flange_a, Steering)
      annotation (Line(points={{-8,10},{54,10},{54,100}}, color={0,0,0}));
    connect(revolute1.flange_a, Steering)
      annotation (Line(points={{-8,50},{54,50},{54,100}}, color={0,0,0}));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
          coordinateSystem(preserveAspectRatio=false)));
  end Chassis2D;
  ...
  model Body2D
    ...
    PlanarMechanics.Parts.Body body(m=1376, I=1137) annotation (Placement(
          transformation(
          extent={{-10,-10},{10,10}},
          rotation=90,
          origin={10,56})));
    PlanarMechanics.Sources.QuadraticSpeedDependentForce
      quadraticSpeedDependentForce1(
      F_nominal=-35.478,
      v_nominal=2.7777777777778,
      tau_nominal=0,
      w_nominal=1)
      annotation (Placement(transformation(extent={{80,-10},{60,10}})));
  ...
  equation
    connect(frame, body.frame_a)
      annotation (Line(points={{-84,0},{10,0},{10,46}}, color={0,0,0}));
    connect(quadraticSpeedDependentForce1.frame_b, body.frame_a) annotation (
        Line(
        points={{60,0},{10,0},{10,46}},
        color={95,95,95},
        thickness=0.5));
    connect(absoluteVelocity.frame_a, frame) annotation (Line(
        points={{-6,-34},{-46,-34},{-46,0},{-84,0}},
        color={95,95,95},
        thickness=0.5));
    connect(absoluteVelocity.v[1], speed) annotation (Line(points={{15,-34.6667},
            {32,-34.6667},{32,-34},{50,-34},{50,-38},{106,-38}}, color={0,0,127}));
    connect(absolutePosition.frame_a, frame) annotation (Line(
        points={{-14,84},{-80,84},{-80,0},{-84,0}},
        color={95,95,95},
        thickness=0.5));
    connect(absolutePosition.r[3], angle) annotation (Line(points={{7,84.6667},
            {42,84.6667},{42,82},{82,82}}, color={0,0,127}));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
          coordinateSystem(preserveAspectRatio=false)));
  end Body2D;
  ...
  model HybrdiCar2D
    ...
    Modelica.Mechanics.Rotational.Components.Fixed fixed(phi0(displayUnit="deg")=
           -0.43633231299858)
  ...
  equation
    ...
    connect(electricDrive.u, gain.y) annotation (Line(points={{-36.4,14},{-42,
            14},{-42,0},{-43.6,0}}, color={0,0,127}));
    connect(electricDrive.flange_a, driveTrain2D.flange_a) annotation (Line(
          points={{-16.2,14},{-8,14},{-8,0},{2,0}}, color={0,0,0}));
    connect(combustionEngineDrive.flange_a, driveTrain2D.flange_a) annotation (
        Line(points={{-16,-18},{-8,-18},{-8,0},{2,0}}, color={0,0,0}));
    connect(driveTrain2D.VL, chassis2D.VL)
      annotation (Line(points={{22,6},{26,6}}, color={0,0,0}));
    connect(driveTrain2D.VR, chassis2D.VR)
      annotation (Line(points={{22,2},{26,2}}, color={0,0,0}));
    connect(driveTrain2D.HL, chassis2D.HL)
      annotation (Line(points={{22,-2},{26,-2}}, color={0,0,0}));
    connect(driveTrain2D.HR, chassis2D.HR)
      annotation (Line(points={{22,-6},{26,-6}}, color={0,0,0}));
    connect(chassis2D.frame, body2D.frame)
      annotation (Line(points={{44,0},{59.6,0}}, color={0,0,0}));
    connect(body2D.speed, driver.car_speed) annotation (Line(points={{78.6,-3.8},
            {96,-3.8},{96,-40},{-92,-40},{-92,-3},{-78.4,-3}}, color={0,0,127}));
    connect(body2D.angle, driver.driving_angle) annotation (Line(points={{76.2,
            8.2},{98,8.2},{98,-44},{-96,-44},{-96,8.4},{-73.6,8.4}}, color={0,0,
            127}));
    connect(chassis2D.Steering, fixed.flange) annotation (Line(points={{41.4,10},
            {41.4,68},{14,68},{14,56}}, color={0,0,0}));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
          coordinateSystem(preserveAspectRatio=false)),
      experiment(StopTime=100, __Dymola_Algorithm="Dassl"));
  end HybrdiCar2D;
  ...
  model DifferentialGearTest
    ...
    Modelica.Mechanics.Rotational.Components.Inertia inertia1(J=100)
      annotation (Placement(transformation(extent={{30,-6},{50,14}})));
    Modelica.Mechanics.Rotational.Components.Fixed fixed
      annotation (Placement(transformation(extent={{62,-42},{82,-22}})));
  equation
    connect(differential.flange_a, constantTorque.flange)
      annotation (Line(points={{-10,13.8},{-10,58},{-38,58}}, color={0,0,0}));
    connect(inertia.flange_b, differential.flange_b)
      annotation (Line(points={{-44,4},{-20,4}}, color={0,0,0}));
    connect(differential.flange_b1, inertia1.flange_a)
      annotation (Line(points={{0,4},{30,4}}, color={0,0,0}));
    connect(inertia1.flange_b, fixed.flange) annotation (Line(points={{50,4},{
            62,4},{62,-32},{72,-32}}, color={0,0,0}));
    annotation (Icon(coordinateSystem(preserveAspectRatio=false)), Diagram(
          coordinateSystem(preserveAspectRatio=false)));
